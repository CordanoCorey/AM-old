...
export class MemberFormComponent extends FormComponent implements OnInit {
  ...
  get valueChanges(): Subscription {
    return this.form.valueChanges.subscribe(value => {
      this.changes.emit(this.modelValue);
    });
  }
  ...
  onAcceptMember() {
    this.acceptMember.emit(this.modelValue);
  }

  onRejectMember() {
    this.rejectMember.emit(this.modelValue);
  }

  onCancelRequest() {
    this.cancelRequest.emit(this.modelValue);
  }
  ...
  onSubmit(e: any) {
    ...
    switch (this.action) {
      case 'ADD_EXISTING_USER':
        this.addExisting.emit(this.modelValue);
        break;
      case 'ADD_NEW_USER':
        this.addNew.emit(this.modelValue);
        break;
      case 'UPDATE_EXISTING_MEMBER':
        this.update.emit(this.modelValue);
        break;
    }
  }
