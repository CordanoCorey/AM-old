...
import { RouterState, RouterActions } from '@caiu/router';
import { Store, SmartComponent } from '@caiu/store';
import { BehaviorSubject } from 'rxjs/BehaviorSubject';

import { Agenda } from '../agendas/agendas.model';
import { AgendasActions, agendaSelector, AgendaActions } from '../agendas/agendas.reducer';
...
export class AgendaTreeviewComponent extends SmartComponent implements OnInit, OnChanges {
  ...
  get active$(): Observable<boolean> {
    return this.agendaId$.map(id => id === this.activeId);
  }
  ...
  get agendaId$(): Observable<number> {
    return this.agenda$.map(agenda => agenda.id).distinctUntilChanged();
  }
  ...
  get editing$(): Observable<boolean> {
    return this.routeName$.map(routeName => routeName === 'agenda-item-edit');
  }
  ...
  get routeName$(): Observable<string> {
    return this.route$.map(route => route.activatedRouteName).distinctUntilChanged();
  }
