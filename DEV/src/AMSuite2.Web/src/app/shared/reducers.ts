...
import { GroupsActions, GroupActions } from '../core/groups/groups.reducer';
...
export function usersReducer(state: Users = new Users(), action: Action): Users {
    switch (action.type) {
        ...
        case GroupActions.GET:
            const members: User[] = (<Group>action.payload).members ?
                action.payload.members.map((member: GroupMember) => member.user) : [];
            return build(Users, state.update(members));
